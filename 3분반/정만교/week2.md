...
2. 4
3. 4
  1. 센서 오류로 무작위로 데이터가 누락된 경우 → MAR
  2. 야간에는 측정이 어려워 값이 자주 누락되며, 시간대 정보는 관측됨 → MNAR
  3. 고소득자가 소득 응답을 의도적으로 회피하는 경우 → MCAR
  4. 무작위 통신 오류로 특정 관측치가 누락된 경우 → MCAR
  5. 설문 응답자가 민감한 질문을 회피하여 결측이 발생한 경우 → MAR